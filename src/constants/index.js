import {
    mobile,
    backend,
    creator,
    web,
    javascript,
    typescript,
    html,
    css,
    reactjs,
    redux,
    tailwind,
    nodejs,
    mongodb,
    git,
    figma,
    docker,
    meta,
    starbucks,
    tesla,
    shopify,
    portfolio1,
    portfolio3,
    portfolio2,
    threejs,
  } from "../assets";
  
  export const navLinks = [
    {
      id: "about",
      title: "About",
    },
    {
      id: "work",
      title: "Work",
    },
    {
      id: "contact",
      title: "Contact",
    },
  ];
  
  const services = [
    {
      title: "Web Developer",
      icon: web,
    },
    {
      title: "React Developer",
      icon: mobile,
    },
    {
      title: "Content Creator",
      icon: creator,
    },
  ];
  
  const technologies = [
    {
      name: "HTML 5",
      icon: html,
    },
    {
      name: "CSS 3",
      icon: css,
    },
    {
      name: "JavaScript",
      icon: javascript,
    },
    {
      name: "React JS",
      icon: reactjs,
    },
    {
      name: "Redux Toolkit",
      icon: redux,
    },
    {
      name: "Tailwind CSS",
      icon: tailwind,
    },
    {
      name: "Node JS",
      icon: nodejs,
    },
    {
      name: "MongoDB",
      icon: mongodb,
    },
    {
      name: "Three JS",
      icon: threejs,
    },
    {
      name: "git",
      icon: git,
    },
    {
      name: "figma",
      icon: figma,
    }
  ];
  
  const experiences = [
    {
      title: "React.js Developer",
      company_name: "Petly",
      icon: starbucks,
      iconBg: "#383E56",
      date: "April 2023",
      points: [
        "Developing and maintaining web application using JavaScript, CSS, HTML, React.js and other related technologies.",
        "Collaborating with cross-functional teams including designers, product managers, and other developers to create high-quality products.",
        "Implementing responsive design and ensuring cross-browser compatibility.",
        "Participating in code reviews and providing constructive feedback to other developers.",
      ],
    },
    {
      title: "Web Developer",
      company_name: "Filmoteka ",
      icon: shopify,
      iconBg: "#383E56",
      date: "Jan 2022",
      points: [
        "Developing and maintaining web application using JavaScript, CSS, HTML and other related technologies.",
        "Collaborating with cross-functional teams including designers, product managers, and other developers to create high-quality products.",
        "Implementing responsive design and ensuring cross-browser compatibility.",
        "Participating in code reviews and providing constructive feedback to other developers.",
      ],
    },
    {
      title: "Web Developer",
      company_name: "Ice-Cream Galaxy ",
      icon: shopify,
      iconBg: "#383E56",
      date: "Nov 2022",
      points: [
        "Developing and maintaining web application using CSS, HTML technologies.",
        "Collaborating with cross-functional teams including designers, product managers, team leaders and other developers to create high-quality products.",
        "Implementing responsive design and ensuring cross-browser compatibility.",
        "Participating in code reviews and providing constructive feedback to other developers.",
      ],
    }
  ];
  
  const projects = [
    {
      name: "Petly",
      description:
        "Web-based platform that allows users to search, sell dand buy pets from different users. It solves the problem each person faces when he needs a friend",
      tags: [
        {
          name: "react",
          color: "blue-text-gradient",
        },
        {
          name: "mongodb",
          color: "green-text-gradient",
        },
        {
          name: "tailwind",
          color: "pink-text-gradient",
        },
      ],
      image: portfolio3,
      source_code_link: "https://github.com/PeterZabila/petly-team-project/",
      live_code_demo: "https://blackinhub.github.io/petly-team-project-client/"
    },
    {
      name: "Filmoteka",
      description:
        "Web application that enables users to search for movies and TV series with different genres. Users also can view info on the movies and TV series. Initial App render show trending movies",
      tags: [
        {
          name: "react",
          color: "blue-text-gradient",
        },
        {
          name: "restapi",
          color: "green-text-gradient",
        },
        {
          name: "scss",
          color: "pink-text-gradient",
        },
      ],
      image: portfolio2,
      source_code_link: "https://github.com/PeterZabila/filmoteka/",
      live_code_demo: "https://peterzabila.github.io/filmoteka/"
    },
    {
      name: "Ice-Cream Galaxy",
      description:
        "Web App for online ice-cream shop, that allows users to choose products, view its details and reviews",
      tags: [
        {
          name: "nextjs",
          color: "blue-text-gradient",
        },
        {
          name: "supabase",
          color: "green-text-gradient",
        },
        {
          name: "css",
          color: "pink-text-gradient",
        },
      ],
      image: portfolio1,
      source_code_link: "https://github.com/PeterZabila/IceCream-Galaxy/",
      live_code_demo: "https://peterzabila.github.io/IceCream-Galaxy/"
    },
  ];
  
  export { services, technologies, experiences, projects };